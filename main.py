...
def start(message):
    ...
    # Inline buttons (self-explanatory)
    ...
    bot.send_message(
        message.chat.id,
        "",                     # Empty text as you wanted
        reply_markup=inline
    )

# ================================
# BUTTON CALLBACK - DIFFERENT MESSAGE FOR EACH BUTTON
# ================================
...
def callback(call):
    if call.data == "btn1":
        ...
        bot.send_message(call.message.chat.id, "አስተያየትዎን ይላኩ...")   # "send your comment" in Amharic
    
    # Optional: remove the "loading" animation on the button
    bot.answer_callback_query(call.id)

# ================================
# USER MESSAGE → ADMIN (QUESTION OR COMMENT)
# ================================
...
def forward_to_admin(message):
    ...
    # Remove the /start button permanently after first message
    bot.send_message(
        message.chat.id,
        "✅ Question/Comment sent!",
